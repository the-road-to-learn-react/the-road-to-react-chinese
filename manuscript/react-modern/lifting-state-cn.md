...
> 前面我们已经了解了回调处理函数，因为它帮助我们保持一个从 Search 组件到 App 组件的开放通信通道。Search 组件不再管理状态，只是在文本输入到输入框之后，将事件向上传递给 App 组件。你也可以将 `searchTerm` 作为 prop 向下传递，并在 App 或 Search 组件中再次展示。
...
> 通过管理 App 组件中的搜索功能状态，我们最终在将 `list` 传递给 List 组件之前，可以用有状态的 `searchTerm` 来过滤列表：
...
此时， [JavaScript 数组的内置过滤函数](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/filter) 可以用来创建一个新的过滤数组。Filter 函数接受一个函数作为参数，该函数访问数组中的每一项并返回 true 或 false。如果函数返回 true，则表示满足条件，该项将保留在新创建的数组中；如果函数返回 false，则将该项删除。
