...
当使用样式化组件时，你使用 JavaScript 模板字符串的方式与JavaScript函数的使用方式相同。模板字符串之间的所有内容都可以看作是一个参数，而 `styled` 对象则让你可以作为函数访问所有必要的HTML元素（如：div、h1）。一旦函数被调用了样式，它就会返回一个React组件，可以在你的App组件中使用：
