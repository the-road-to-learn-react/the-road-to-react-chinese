...
这应用程序的行为是一样的，因为现在从`useState`返回`stories`，仍然会被过滤到`searchedStories`中，并展示在列表中。接下来，我们将要通过移除列表中的一个item来操作列表：
...
App组件中的回调处理函数将要删除的item作为参数接收，并且会筛选当前stories，根据删除所有不符合条件的items的信息。然后将返回的stories 设置作为新的状态，并且List组件会传递函数给它的子组件。它没有使用这个新的信息，它仅是传递下去：
...
我们可能只能传递item的`objectID`，因为这是我们在App组件的回调处理函数中需要的全部，但是我们不确定这个处理函数之后可能需要什么信息。删除一个item可能需要的不仅是一个标识符。如果我们调用`onRemoveItem`处理函数，这个item应该被传递，不仅仅是它的标识符。
...
可以接受三个处理函数版本中，其中二个是内联和一个常规处理函数。非内联的处理函数会移动详细的实现进入函数组件块体，内联函数会移动详细的实现在JSX中。
